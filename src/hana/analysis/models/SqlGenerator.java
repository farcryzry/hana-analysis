...
public class SqlGenerator {
	...
	public static String createTableType(String typeName, LinkedHashMap<String, String> columns) {
		addToObjectList(typeName);
		String sql = String.format("CREATE TYPE %s AS TABLE (%s);", typeName, mapToString(columns));
		return sql;
	}
	
	public static String createTable(String tableName, LinkedHashMap<String, String> columns) {
		addToObjectList(tableName);
		String sql = String.format("CREATE COLUMN TABLE %s (%s);", tableName, mapToString(columns));
		return sql;
	}
	
	public static String createTableByType(String tableName, String type) {
		addToObjectList(tableName);
		String sql = String.format("CREATE COLUMN TABLE %s LIKE %s;", tableName, type);
		return sql;
	}
	
	public static String createViewByTable(String viewName, String tableName) {
		return createView(viewName, String.format("SELECT * FROM %S", tableName));
	}
	...
	public static String createView(String viewName, String viewDefination) {
		...
		String sql = String.format("CREATE VIEW %s AS %S;", viewName, viewDefination);
		return sql;
	}
	
	public static String insert(String tableName, List<Object> values) {
		addToObjectList(tableName);
		String sql = String.format("INSERT INTO %s VALUES (%s);", tableName, listToString(values, true));
		return sql;
	}
	...
	public static String callProcedure(String procedureName, List<Object> values) {
		return callProcedure(procedureName, values, false);
	}
	
	public static String callProcedure(String procedureName, List<Object> values, boolean withOverview) {
		String sql = String.format("CALL %s (%s) %s;", procedureName, listToString(values), withOverview ? "WITH OVERVIEW" : "");
		return sql;
	}
	
	public static String update(String tableName, String setClause, String conditionClause) {
		
		String sql = String.format("UPDATE %s SET %s WHERE %s;", tableName, setClause, conditionClause);
		return sql;
	}
